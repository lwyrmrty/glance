...
`);E=$.pop()||"";for(let Z of $){let S=Z.trim();if(!S||!S.startsWith("data: "))continue;let j=S.slice(6);if(j!=="[DONE]")try{let I=JSON.parse(j);if(I.content){T+=I.content;if(!H._renderTimer){H._renderTimer=setTimeout(()=>{H.innerHTML=V(T);o.scrollTop=o.scrollHeight;H._renderTimer=null},80)}}}catch{}}}if(H._renderTimer){clearTimeout(H._renderTimer);H._renderTimer=null}H.innerHTML=V(T);if(W){H.querySelectorAll("[data-glance-tab]").forEach(lnk=>{lnk.addEventListener("click",ev=>{ev.preventDefault();let tn=lnk.getAttribute("data-glance-tab").toLowerCase().trim(),ti=W.config.tabs.findIndex(t=>{let g=t.name.toLowerCase().replace(/\s+/g,"-");return t.name.toLowerCase().trim()===tn||g===tn||(t.hash_trigger||"").toLowerCase()===tn});if(ti>=0)W.switchTab(ti)})})}o.scrollTop=o.scrollHeight;f.push({role:"assistant",content:T});(async function(){try{if(!_chatSessionId&&!_chatPending){_chatPending=!0;let cr=await fetch(c+'/api/widget-chats',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'create_session',widget_id:s,session_id:_glanceSessionId,widget_user_id:_glanceUserId,tab_name:l.name||''})});if(cr.ok){let cd=await cr.json();_chatSessionId=cd.chat_session_id}_chatPending=!1}if(_chatSessionId){fetch(c+'/api/widget-chats',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'add_messages',chat_session_id:_chatSessionId,messages:[{role:'user',content:m},{role:'assistant',content:T}]})}).catch(function(){})}}catch(e){}})()}catch(C){if(y.remove(),C.name!=="AbortError"){let v=_sv(l.failure_message||"Sorry, something went wrong.");x("assistant",v),console.error("[Glance] Chat error:",C)}}finally{h=!1,g.disabled=!1,u=null,a.focus()}}return r&&(a.value=r,b()),()=>{u&&u.abort()}}function V(l){if(!l)return"";let e=O(l);return e=e.replace(/```(\w*)\n([\s\S]*?)```/g,(p,c,r)=>`<pre><code>${r.trim()}</code></pre>`),e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/__(.+?)__/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/(?<!\w)_([^_]+)_(?!\w)/g,"<em>$1</em>"),e=e.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(p,c,r)=>{if(r.startsWith("#")&&r.length>1){let tabName=r.slice(1);return`<a href="#" data-glance-tab="${tabName}" class="glance-tab-link">${c}</a>`}try{let u=new URL(r,window.location.origin);return u.origin===window.location.origin?`<a href="${r}" target="_top">${c}</a>`:`<a href="${r}" target="_blank" rel="noopener noreferrer">${c}</a>`}catch{return`<a href="${r}" target="_blank" rel="noopener noreferrer">${c}</a>`}}),e=e.replace(/^### (.+)$/gm,"<h3>$1</h3>"),e=e.replace(/^## (.+)$/gm,"<h2>$1</h2>"),e=e.replace(/^# (.+)$/gm,"<h1>$1</h1>"),e=e.replace(/^---$/gm,"<hr>"),e=e.replace(/\n\n+(?=\d+\. )/g,"\n"),e=e.replace(/\n\n+(?=[-*] )/g,"\n"),e=e.replace(/^(?:[-*]) (.+)$/gm,"<li>$1</li>"),e=e.replace(/((?:<li>.*<\/li>\n?)+)/g,"<ul>$1</ul>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,p=>p.includes("<ul>")?p:`<ol>${p}</ol>`),e=e.replace(/<\/ol>((?:(?!<h[1-6])[\s\S])*?)<ol>/g,'$1'),e=e.split(/\n\n+/).map(p=>{let c=p.trim();return c?/^<(h[1-6]|ul|ol|li|pre|hr|blockquote)/.test(c)?c:`<p>${c.replace(/\n/g,"<br>")}</p>`:""}).join(""),e}function O(l){return l.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}/* ---- Embed (Tally) Tab Renderer ---- */
function J(l,e,W){var url=l.embed_url;if(!url){e.innerHTML='<div class="glance-placeholder">No embed URL configured for this tab.</div>';return null}url=url.replace(/([?&])transparentBackground=1(&)?/g,function(_,sep,amp){return amp?sep:""}).replace(/[?&]$/,"");var w=document.createElement("div");w.className="widget-content embed";var p=document.createElement("div");p.className="embed-portal-placeholder";p.style.cssText="position:relative;flex:1;min-height:0;";w.appendChild(p);e.appendChild(w);var f=document.createElement("iframe");f.setAttribute("frameborder","0");f.setAttribute("allow","payment; fullscreen; clipboard-write");f.setAttribute("title",l.name||"Embedded content");f.style.cssText="border:0;display:block;position:fixed;inset:0;width:0;height:0;z-index:2147483647;background:#fff;visibility:hidden;opacity:0;pointer-events:none;";document.body.appendChild(f);var r=null;var ro=null;var mo=null;var lastRect=null;var srcSet=!1;function u(){if(!p.isConnected){f.style.visibility="hidden";f.style.opacity="0";f.style.pointerEvents="none";return}var rect=p.getBoundingClientRect();if(rect.width>0&&rect.height>0){lastRect=rect;if(!srcSet){f.src=url;srcSet=!0}}if(!lastRect){f.style.visibility="hidden";f.style.opacity="0";f.style.pointerEvents="none";return}f.style.left=lastRect.left+"px";f.style.top=lastRect.top+"px";f.style.width=lastRect.width+"px";f.style.height=lastRect.height+"px";var panelOpen=!0;if(W&&W._panel)panelOpen=W._panel.classList.contains("open");if(!panelOpen){f.style.visibility="hidden";f.style.opacity="0";f.style.pointerEvents="none";return}f.style.visibility="visible";f.style.opacity="1";f.style.pointerEvents="auto";f.offsetHeight}function h(){if(W&&W._panel){W._panel.getBoundingClientRect();W._panel.offsetHeight}p.getBoundingClientRect();p.offsetHeight;f.getBoundingClientRect();f.offsetHeight}function b(){var s=Date.now();(function t(){u();h();if(Date.now()-s<1500)requestAnimationFrame(t)})()}u();b();window.addEventListener("resize",u);window.addEventListener("scroll",u,!0);if(window.ResizeObserver){ro=new ResizeObserver(u);ro.observe(p)}if(window.MutationObserver&&W&&W._panel){mo=new MutationObserver(function(){u();b()});mo.observe(W._panel,{attributes:!0,attributeFilter:["class"]})}if(W&&!W._embedPatched){W._embedPatched=!0;var _open=W.open,_switch=W.switchTab;W.open=function(){var res=_open.apply(W,arguments);u();b();return res};W.switchTab=function(){var res=_switch.apply(W,arguments);u();b();return res}}r=setInterval(u,200);return function(){window.removeEventListener("resize",u);window.removeEventListener("scroll",u,!0);if(ro)ro.disconnect();if(mo)mo.disconnect();if(r)clearInterval(r);if(f&&f.parentNode)f.parentNode.removeChild(f)}}
/* ---- Glance Analytics ---- */
